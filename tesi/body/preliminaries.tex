...
\noindent Per esempio, assumendo una descrizione abbastanza simile alla realtà moderna, se scriviamo una query che richiede tutti i nodi che appartengono ad un concetto sia di Pizza che di Gelato, ovviamente questa query non potrà mai essere abitata, perchè non esistono pizze che allo stesso tempo sono anche dei gelati. Il concetto di abitabilità però non è da confondere con il fatto che essa sia abitata: una query potrebbe essere abitata, ma nel grafo RDF su cui stiamo lavorando non c'è nessun nodo che soddisfa i requisiti. Per approfondire sulle query congiuntive, sulla loro semantica e abitabilità, i lettori interessati possono usare \cite{baader2017introductionDL} come ottimo punto d'inizio.
\begin{figure}[h]
	\centering
